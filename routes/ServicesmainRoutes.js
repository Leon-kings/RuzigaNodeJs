...
// CREATE - Create new form submission
router.post('/', validate(validateFormData), mainController.createFormData);

// READ - Get all form submissions with pagination and filtering
router.get('/', mainController.getAllFormData);

// READ - Get single form submission
router.get('/email/:email', mainController.getFormDataByEmail);
router.get('/:id', mainController.getFormDataById);

// UPDATE - Update form submission
router.put('/:id', validate(validateFormData), mainController.updateFormData);
...
router.get('/dashboard/performance', async (req, res) => {
  try {
    const performance = await require('../models/ServicesFormData').aggregate([
      { $match: { status: { $in: ['contacted', 'approved', 'rejected'] } } },
      { $addFields: {
          responseTime: {
            $divide: [
              { $subtract: ['$updatedAt', '$submittedAt'] },
...
router.get('/export/csv', async (req, res) => {
  try {
    ...
    // Convert to CSV
    const headers = [
      'ID','Full Name','Email','Phone','Target Country','Program',
      'Start Date','Education Level','Budget','Status','Submitted At'
    ].join(',');
