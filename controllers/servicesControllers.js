...
// // Email sending functions - ALWAYS SEND EMAILS
// const sendEmail = async (to, subject, html, attachments = []) => {
//   try {
...
//     // Get current booking first to know old status
...
const sendAdminNotification = async (booking) => {
  try {
    ...
    if (adminEmails.length === 0) {
      console.log("⚠️ No admin emails configured - cannot send admin notification");
    ...
    return { 
      ...
      results,
      summary: results.map(r => ({ email: r.to, sent: r.success, error: r.error }))
    };
  } catch (error) {
    console.error("❌ Error in sendAdminNotification:", error.message);
    return { success: false, error: error.message };
...
exports.createBooking = async (req, res) => {
  try {
    ...
    const confirmationResult = emailResults[0].status === 'fulfilled' 
      ? emailResults[0].value 
      : { success: false, error: emailResults[0].reason?.message };
    
    const adminResult = emailResults[1].status === 'fulfilled' 
      ? emailResults[1].value 
      : { success: false, error: emailResults[1].reason?.message };

    console.log(
      `📧 Email results for booking ${booking._id}:`,
      `Confirmation: ${confirmationResult.success ? 'sent' : 'failed'},`,
    ...
    return res.status(201).json({
      ...
      emails: {
        ...
        admin: {
          ...
          details: adminResult.results || null
        }
      },
      data: {
        booking,
...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    // Send status update email if status changed (await the result)
    let emailResult = null;
    if (status !== oldStatus && status !== "cancelled") {
      emailResult = await sendStatusUpdate(booking, status);
      console.log(`📧 Status update email ${emailResult.success ? 'sent' : 'failed'} for booking ${booking._id}`);
    }
...
exports.deleteBooking = async (req, res) => {
  try {
    ...
    // Send cancellation email and wait for result
    const emailResult = await sendCancellationEmail(booking);
    console.log(`📧 Cancellation email ${emailResult.success ? 'sent' : 'failed'} for booking ${booking._id}`);
