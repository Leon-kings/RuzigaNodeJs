...
const emailService = {
  ...
  sendBookingConfirmation: async (booking, visaCatalog = null) => {
    ...
    if (!booking.booking.bookingReference) {
      booking.booking.bookingReference = bookingRef;
      await booking.save().catch(err => console.error('Failed to save booking reference:', err));
    }
...
exports.createVisaCatalog = async (req, res) => {
  try {
    ...
    console.error('❌ Create visa catalog error:', err);
    res.status(400).json({ success: false, message: err.message });
  }
};
...
exports.getVisaCatalogs = async (req, res) => {
  try {
    ...
    console.error('❌ Get visa catalogs error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.getVisaCatalogById = async (req, res) => {
  try {
    ...
    console.error('❌ Get visa catalog by ID error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.updateVisaCatalog = async (req, res) => {
  try {
    ...
    console.error('❌ Update visa catalog error:', err);
    res.status(400).json({ success: false, message: err.message });
  }
};
...
exports.deleteVisaCatalog = async (req, res) => {
  try {
    ...
    console.error('❌ Delete visa catalog error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.createBooking = async (req, res) => {
  try {
    ...
    Promise.allSettled([
      ...
      console.log(`📧 Email notifications for booking ${bookingRef}:`, 
...
exports.getBookingsByEmail = async (req, res) => {
  try {
    ...
    const bookings = await VisaService.find({
      recordType: "visa-booking",
      "booking.customer.email": email.toLowerCase(),
    }).sort({ createdAt: -1 });
    ...
    console.error('❌ Get bookings by email error:', err);
    res.status(500).json({
      success: false,
      message: err.message,
    });
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    if (newStatus && oldStatus && newStatus !== oldStatus) {
      emailService.sendStatusUpdate(booking, oldStatus, newStatus)
        .then(result => {
          console.log(`📧 Status update email ${result.success ? 'sent' : 'failed'} for booking ${booking._id}`);
        })
        .catch(err => console.error('❌ Error sending status update email:', err));
    }
...
exports.uploadDocument = async (req, res) => {
  try {
    ...
    // Send document upload confirmation (fire and forget)
    emailService.sendDocumentUploadConfirmation(booking, category, doc)
      .then(result => {
        console.log(`📧 Document upload confirmation ${result.success ? 'sent' : 'failed'} for ${category}`);
      })
      .catch(err => console.error('❌ Error sending document upload confirmation:', err));
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        adminEmail: process.env.ADMIN_EMAIL || 'Not configured',
        companyName: process.env.COMPANY_NAME || 'REC APPLY'
      }
    });
  } catch (err) {
...
// Export email service for potential use elsewhere
