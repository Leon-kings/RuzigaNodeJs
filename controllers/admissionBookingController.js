...
// ====================
  ...
  //   });
  // };
// ⭐ Create Bulletproof Transporter
const createTransporter = () => {
  ...
  return nodemailer.createTransport({
    ...
    pool: true, // ⭐ Prevents connection re-creation (IMPORTANT)
    maxConnections: 5,
    maxMessages: 10,
...
const emailService = {
 ...
 sendEmail : async (to, subject, html, isAdminNotification = false, retries = 3) => {
  try {
    ...
    // ⭐ Retry logic (Bulletproof feature)
    if (retries > 0) {
...
class AdmissionController {
  ...
  async createBooking(req, res) {
    try {
      ...
      if (userEmail) {
        await emailService.sendBookingConfirmation(enrichedBooking, userEmail, userName);
        await emailService.sendAdminNotification('booking', enrichedBooking, userEmail, userName);
      }
  ...
  async createApplication(req, res) {
    try {
      ...
      if (userEmail) {
        await emailService.sendApplicationConfirmation(enrichedApplication, userEmail, userName);
        await emailService.sendAdminNotification('application', enrichedApplication, userEmail, userName);
      }
