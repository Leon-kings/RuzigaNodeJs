...
exports.createBooking = async (req, res) => {
  try {
    ...
    // Send email notifications
    await emailService.sendBookingConfirmation(booking, plane);
    await emailService.sendAdminNotification(booking, plane);
...
exports.updateBooking = async (req, res) => {
  ...
  // Send status update email if status changed
  if (req.body.status && oldStatus !== req.body.status) {
    await emailService.sendStatusUpdate(booking, oldStatus, req.body.status);
    booking.statistics.emailSentCount += 1;
    await booking.save();
  }
