...
const createTransporter = () => {
  ...
  return nodemailer.createTransport({
    ...
    pool: true, // ⭐ Prevents timeout from repeated connections
    maxConnections: 5,
    maxMessages: 10,
...
const emailService = {
 ...
 sendEmail : async (to, subject, html, isAdminNotification = false, retries = 3) => {
  try {
    ...
    // Optional connection verification (does not block sending)
    try {
      await transporter.verify();
      console.log("✅ SMTP connection verified");
    } catch (verifyError) {
      console.log("⚠ SMTP verification warning:", verifyError.message);
    }
...
exports.createBooking = async (req, res) => {
  try {
    ...
    if (customerEmail) {
      await emailService.sendBookingConfirmation(bookingData, customerEmail, customerName);
      await emailService.sendAdminNotification(bookingData, customerEmail, customerName);
    }
