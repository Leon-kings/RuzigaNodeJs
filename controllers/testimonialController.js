  ...
  createTestimonial = async (req, res) => {
    try {
      ...
      if (testimonial.email) {
        ...
        emailResults.user = await this.sendEmail(
          testimonial.email,
          `✨ Thank You for Your Testimonial - ${this.companyName}`,
          userHtml
        );
        console.log(`📧 Confirmation email ${emailResults.user.success ? 'sent' : 'failed'} to ${testimonial.email}`);
      }
      ...
      emailResults.admin = await this.sendEmail(
        this.adminEmail,
        `🔔 New Testimonial Pending Review: ${testimonial.name}`,
        adminHtml
      );
      console.log(`📧 Admin notification ${emailResults.admin.success ? 'sent' : 'failed'}`);
      ...
      res.status(201).json({
        ...
        emails: {
          user: {
            sent: emailResults.user.success,
            error: emailResults.user.error || null
          },
          admin: {
            sent: emailResults.admin.success,
            error: emailResults.admin.error || null
          }
        }
      });
    } catch (error) {
  ...
  updateTestimonial = async (req, res) => {
    try {
      ...
      if (currentTestimonial.status !== testimonial.status && testimonial.email) {
        ...
        console.log(`📧 Status update email ${emailResults.user.success ? 'sent' : 'failed'} to ${testimonial.email}`);
        ...
        console.log(`📧 Admin status update email ${emailResults.admin.success ? 'sent' : 'failed'}`);
      }
  ...
  deleteTestimonial = async (req, res) => {
    try {
      ...
      if (testimonial.email) {
        ...
        console.log(`📧 Deletion notification email ${emailResult.success ? 'sent' : 'failed'} to ${testimonial.email}`);
      }
  ...
  approveTestimonial = async (req, res) => {
    try {
      ...
      if (testimonial.email) {
        ...
        console.log(`📧 Approval email ${emailResults.user.success ? 'sent' : 'failed'} to ${testimonial.email}`);
      }
      ...
      console.log(`📧 Admin approval notification ${emailResults.admin.success ? 'sent' : 'failed'}`);
